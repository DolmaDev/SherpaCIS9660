...
# Ensure top3 exists for your charts (you referenced it later)
top3 = results_df.head(3).copy()

# Keep only the needed columns in a consistent order
_cols = [c for c in ["Model", "RMSE", "MSE", "MAE", "R2"] if c in results_df.columns]
REGRESSION_METRICS = results_df[_cols].copy()

# (Optional) force plain Python types for JSON/Streamlit friendliness
for c in ["RMSE", "MSE", "MAE", "R2"]:
    if c in REGRESSION_METRICS.columns:
        REGRESSION_METRICS[c] = REGRESSION_METRICS[c].astype(float)

# Expose to ModelReports.py
def get_regression_metrics(top_k: int | None = None):
    """
    Returns a list of dicts like:
      [{"Model": "...", "RMSE": ..., "MSE": ..., "MAE": ..., "R2": ...}, ...]
    Sorted by R2 (descending). If top_k is provided, returns only the top-k rows.
    """
